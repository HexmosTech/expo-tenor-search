...
var Utilities = require('./Tools/Utilities');
var Trending = require('./Methods/Trending');
var Categories = require('./Methods/Categories');
var Search = require('./Methods/Search');
function TenorJS(_ref) {
  var Credentials = _ref.Credentials;
  var trending = Trending(Credentials, Utilities);
  var categories = Categories(Credentials, Utilities);
  var search = Search(Credentials, Utilities);
  return {
    trending: trending,
...
var he = require('he');
function GifSearch(_ref) {
  var tenorkey = _ref.tenorkey,
    MediaFilter = _ref.MediaFilter,
    onGifSelect = _ref.onGifSelect;
  var _useState = React.useState(null),
    jsonString = _useState[0],
    setJsonString = _useState[1];
  var _useState2 = React.useState(null),
    selectedCategory = _useState2[0],
    setSelectedCategory = _useState2[1];
  var _useState3 = React.useState(null),
    selectedGif = _useState3[0],
    setSelectedGif = _useState3[1];
  var _useState4 = React.useState(false),
    setPreviousCategory = _useState4[1];
  var myKey = tenorkey;
  var myFilter = MediaFilter;
  var Tenor = TenorJS.client({
    Key: myKey,
    Filter: 'off',
    Locale: 'en_US',
  ...
  React.useEffect(function () {
    if (selectedCategory === 'selectedGif') {
      ...
      Tenor.search.Query(selectedCategory, '30').then(function (Results) {
        htmlPage += "<body>";
        ...
        Results.forEach(function (Category) {
          ...
          htmlPage += "<img src=\"" + Category.media_formats.gif.url + "\" alt=\"Image" + (count + 1) + "\" />";
          htmlPage += "</button></div>";
          count++;
        });
        ...
        var escapedHtml = he.encode(htmlPage).replace(/"/g, '\\"').replace(/\n/g, '');
        var decodedHtml = he.decode(escapedHtml);
        var resultingJson = JSON.stringify({
          html: decodedHtml
        });
        var jsonResults = JSON.parse(resultingJson);
        setJsonString(jsonResults['html']);
      })["catch"](console.error);
    } else {
      Tenor.categories.List('emoji').then(function (Results) {
        htmlPage += '<body> ';
        htmlPage += '<div class="row">';
        var count = 0;
        Results.forEach(function (Category) {
          htmlPage += "<div class=\"column\"><button onclick=\"sendDataToReactNativeApp('" + Category.searchterm + "')\"\n        >";
          htmlPage += "<img src=\"" + Category.image + "\" alt=\"Image" + (count + 1) + "\" style=\"filter: brightness(50%);\"/>";
          htmlPage += "<div class=\"gif-text\">" + Category.searchterm + "</div></button></div>";
          count++;
        });
        htmlPage += '</div> <script>    const sendDataToReactNativeApp = async (data) => {    window.ReactNativeWebView.postMessage(data);    };    </script> </body>  </html>  ';
        var escapedHtml = he.encode(htmlPage).replace(/"/g, '\\"').replace(/\n/g, '');
        var decodedHtml = he.decode(escapedHtml);
        var resultingJson = JSON.stringify({
          html: decodedHtml
  ...
  var onCloseButtonPress = function onCloseButtonPress() {
    setSelectedCategory(null);
  };
  return /*#__PURE__*/React__default.createElement(reactNative.View, {
  ...
  }, /*#__PURE__*/React__default.createElement(reactNative.TextInput, {
    style: {
      flex: 1,
      height: 40,
      paddingLeft: 10
    ...
    onChangeText: function onChangeText(text) {
      return setSelectedCategory(text);
    },
    value: selectedCategory
  }), selectedCategory ? /*#__PURE__*/React__default.createElement(reactNative.TouchableOpacity, {
  ...
  }, "X")) : null)), jsonString ? /*#__PURE__*/React__default.createElement(reactNativeWebview.WebView, {
    originWhitelist: ['*'],
    source: {
      html: jsonString
    },
  ...
  }) : /*#__PURE__*/React__default.createElement(reactNative.Text, null, "Loading the Gifs..."));
}
