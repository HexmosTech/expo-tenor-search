...
function GifSearch({
  ...
  useEffect(() => {
    if (selectedCategory === 'selectedGif') {
      ...
      Tenor.search.Query(selectedCategory, '30').then(Results => {
        htmlPage += `<body>`;
      ...
      >`;
        ...
        const escapedHtml = he.encode(htmlPage).replace(/"/g, '\\"').replace(/\n/g, '');
        const decodedHtml = he.decode(escapedHtml);
        const resultingJson = JSON.stringify({
          html: decodedHtml
        });
      ...
      Tenor.categories.List('emoji').then(Results => {
        htmlPage += '<body> ';
        htmlPage += '<div class="row">';
        let count = 0;
        Results.forEach(Category => {
  ...
  }), selectedCategory ? /*#__PURE__*/React.createElement(TouchableOpacity, {
  ...
  }, "X")) : null)), jsonString ? /*#__PURE__*/React.createElement(WebView, {
    originWhitelist: ['*'],
    source: {
      html: jsonString
    },
  ...
  }) : /*#__PURE__*/React.createElement(Text, null, "Loading the Gifs..."));
}
